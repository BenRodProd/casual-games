...
export default function Quiz({ setGamePoints }) {
  ...
  function handleWrongAnswer() {
    ...
    if (language === 'english') {
      setPopupMessage(
        `Wrong! The correct answer is "${currentCard.answers[currentCard.correctAnswerIndex]}"`
      );
    } else if (language === 'german') {
      setPopupMessage(
        `Falsch! Die richtige Antwort ist "${currentCard.answers[currentCard.correctAnswerIndex]}"`
      );
    }
    setPopup(true);
  }
  ...
  return (
    <>
      <GameWrapper>
        ...
        <Card>
          <CardBackground
            src={`/assets/quiz/${currentCard.topic}.jpg`}
            alt={currentCard.topic}
            width={200}
            height={100}
          />
          ...
          {currentCard.answers.map((answer, index) => {
            return (
              <ChooseAnswer key={index} onClick={() => handleAnswer(index)}>
                {answer}
              </ChooseAnswer>
            );
          })}
        ...
        {popup && (
          <Popup onClick={() => handleNextQuestion()}>
            <p>{popupMessage}</p>
          </Popup>
        )}
      </GameWrapper>
